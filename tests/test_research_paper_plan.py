...
class ResearchPaperPlanTests(unittest.TestCase):  # noqa: F405
    ...
    def test_research_plan_outputs_chinese_executor_files(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertIn("mathematical formulation or proof issue → runtime math reviewer", prompt)
            self.assertIn("baseline reproduction → runtime reproduction worker", prompt)
            self.assertIn("cross-file consistency check → `/research audit` or runtime reviewer", prompt)
