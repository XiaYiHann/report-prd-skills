...
class ResearchGoalTests(unittest.TestCase):  # noqa: F405
    ...
    def test_research_goal_refreshes_goal_md_and_goal_lock(self) -> None:
        ...
        self.assertIn("## Drift Repair Then Execute Policy", goal_text)
        self.assertIn("repair_then_execute", goal_text)
        self.assertIn("latest approved design source", goal_text)
        self.assertIn("do not stop after a repair-only pass", goal_text)
        self.assertIn("continue executing the runnable task set", goal_text)
        self.assertIn("run `goal-ready` after the drift repair", goal_text)
        self.assertIn("RQ 调度真源是 `RESEARCH_SPINE.yaml`", goal_text)
        self.assertIn("单个 RQ 的执行真源是 `rqs/RQxx/TASKS.yaml`", goal_text)
        self.assertIn("`TASK_QUEUE.yaml` 只是兼容聚合视图", goal_text)
        ...
        self.assertIn("runtime literature worker", goal_text)
        self.assertIn("runtime analysis reviewer", goal_text)
        self.assertIn("`/research audit` or runtime reviewer", goal_text)
        self.assertIn("main controller remains responsible for state updates", goal_text)
