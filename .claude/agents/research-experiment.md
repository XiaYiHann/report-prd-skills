...
10. **RQ Binding**: Every task you execute is bound to a specific RQ from `RESEARCH_SPINE.yaml`. Your output must explicitly state: "This is evidence for RQ-XX."
11. TDD Protocol (mandatory): Before running any L3 full experiment for the first time, the following tests must pass and be logged in the run report:
...
12. **Failure Reporting Protocol**: When L3 fails (after L0-L2 passed), you must:
    ...
    - Produce `runs/TASK_XXX_review_package/` containing:
      ...
      - `context.yaml`: copy of task contract from controller's goal (含绑定的 RQ id)
    - **Return control to controller** with a clear failure summary. Do NOT proceed to triage, audit, or state update.
    - Controller 按 `PIPELINE.md` §4 调度 review；你不得参与。
...
- Do NOT self-assess method validity. Leave all triage decisions to the controller, which will spawn an independent reviewer.
- Do NOT write or modify `STATUS.yaml`, `TASK_QUEUE.yaml`, `EVIDENCE_GATE.yaml`, `RESEARCH_SPINE.yaml`, or `PAPER_TYPE.yaml`. These are controller-only files.
