...
`research-execution-skills` 是一组面向 Codex / Claude Code 的研究执行技能。它不是独立服务端系统，也不提供独立常驻 backend；Codex / Claude Code 是实际执行 `RESEARCH_SPINE.yaml` 中 non-final RQ 及其 `rqs/RQxx/TASKS.yaml` 的 agent executor，`TASK_QUEUE.yaml` 只保留为扁平兼容聚合视图；技能族负责提供文件系统协议、状态提交入口、证据记录格式和审计门禁。
...
```text
Direction -> Goal -> RQ Spine -> Evidence/Audit -> Wiki/Closeout
```
...
3. 先跑什么：用 `research-status` 做只读状态检查，再从 `RESEARCH_SPINE.yaml` 中选择 runnable RQ，并执行对应 `rqs/RQxx/TASKS.yaml`；`TASK_QUEUE.yaml` 仅作扁平索引。
...
核心原则：`RESEARCH_DIRECTION.md` 控制探索边界；`/research explore` 负责纯探索；当前 `Vn/PRD.tex` 是当前 epoch 的人类研究真源，并真实渲染为 `Vn/PRD.pdf`；`Vn/PRD_SUMMARY.md` 只是 agent context，不得反向覆盖 PRD；`Vn/RESEARCH_SPINE.yaml` 是版本级 RQ 调度与证据链绑定合同（RQ → Claim → Experiment → Evidence → Figure/Table → Paper Section）；`Vn/rqs/RQxx/SPEC.yaml`、`PLAN.md` 与 `TASKS.yaml` 是 RQ-local 执行真源；`Vn/TASK_QUEUE.yaml` 只是兼容性的全局聚合视图；`Vn/EVIDENCE_GATE.yaml` 负责 claim admission；`update_state.py` 在每次任务完成后原子更新状态；Git 记录真实工程变化；Paper 只是表达层，不能反推实验、数据集、基线、指标、seed、任务、harness 或结果。
...
安装器以 `~/.agents/skills` 为唯一安装真源，并把 `~/.claude/skills` 整体收敛为指向它的 symlink。`--force` 会先删除 research-loop 管理过的历史入口，再安装当前 manifest；`research-prd`、`research-ppt`、`research-evidence`、`research-writing` 会整目录删除，`research-spec`、`research-plan`、`research-paper` 只保留内部脚本且不得包含 `SKILL.md`。当前仓库管理的 Claude project agent surface 只安装 `research-experiment.md`；其它 specialist worker / reviewer 视为 runtime-provided capabilities，而不是本仓库必须分发的本地模板。
...
Research Direction
  -> V0 PRD.tex / PRD.pdf
  -> V0 RESEARCH_SPINE.yaml
  -> V0 RQ-local Spec
  -> V0 RQ-local Plan
  -> V0 RQ-local Tasks
  -> V0 Task Queue (compatibility view)
  -> Claude/Codex executes runnable RQ task
  -> Gate
  -> Wiki
  -> Closeout
  -> V1 PRD
...
.claude/
  agents/
    research-experiment.md    # Claude Code project-level experiment worker template
```
...
初始化后，`RESEARCH_SPINE.yaml` 会先声明当前版本的初始 RQ scaffold，`TASK_QUEUE.yaml` 只同步一个兼容性的 flat active task 便于检查。AI 与用户逐章讨论 16 章 PRD，AI 填写所有内容：
...
当 `RESEARCH_SPINE.yaml` 与 `rqs/RQxx/TASKS.yaml` 中出现具体执行任务后，进入持续循环：

```
while STATUS.yaml.status not in (closed_*, gate_blocked):
    1. 读取 `RESEARCH_SPINE.yaml` 中所有 non-final RQ
    2. 对每个 runnable RQ 读取 `rqs/RQxx/TASKS.yaml` 中的下一步任务
    3. 执行 bounded RQ task（写代码/跑实验/复现 baseline）
    4. 完成后运行 update_state.py 原子更新 `RESEARCH_SPINE.yaml`、RQ-local `TASKS.yaml` 与兼容聚合视图 `TASK_QUEUE.yaml`
    5. 若某个 RQ blocked，仅冻结该 RQ；其余 runnable RQ 继续
```

**停止条件**：`gate_blocked`（报告 blocker 等待人工决策）、`closed_*`（报告 closeout 摘要）、实验证据反驳 PRD 核心假设（写 negative_result 请求 review）。

若要在 Codex 或 Claude Code 中驱动自动执行，使用当前版本的 `docs/research/{CURRENT}/goal.md` 作为目标模式输入；RQ 调度真源是 `RESEARCH_SPINE.yaml`，单个 RQ 的执行真源是 `rqs/RQxx/TASKS.yaml`，`TASK_QUEUE.yaml` 仅作兼容聚合视图。`goal.md` 必须暴露版本级 task dependency graph、可运行任务集合与 blocked-branch 传播规则；当执行器支持并行时，可并行推进正交 runnable RQ tasks。
...
`TASK_QUEUE.yaml` 是 Gate-aware 的兼容聚合队列；它为 status / audit / legacy validators 提供 flat next-action 视图，但不得覆盖 `RESEARCH_SPINE.yaml` 与 `rqs/RQxx/TASKS.yaml` 的真实调度语义。短规则：
...
| **Worker** | 执行分配任务（coding / experiment / analysis / literature / math），产出证据 | runtime-provided specialist worker；本仓库只分发 `research-experiment` 模板 |
| **Reviewer** | 按需 spawn，只读日志，判断失败是否影响 RQ 核心假设，不产出代码 | runtime-provided reviewer，或 `/research audit` / `research-audit` skill |
...
在 goal mode 中，subagent 是优先执行路径，不是可有可无的装饰。`prefer_subagents`: 优先使用 subagent 执行 literature、reproduction、coding、experiment、analysis、math、paper、audit 等 bounded specialist work；除非任务过小、没有匹配 runtime specialist、或下一步被主 controller 的状态更新阻塞，否则不要把这些工作全部留在主会话内完成。main controller remains responsible for state updates、gate decisions、final evidence admission、`update_state.py`、`RESEARCH_SPINE.yaml`、RQ-local `TASKS.yaml`、兼容聚合视图 `TASK_QUEUE.yaml` 与 `STATUS.yaml`。subagent 的输出必须列出命令、改动文件、artifact/hash 与残余 blocker；prompt-only subagent note 不能充当实验或 claim evidence。
...
**阶段 2：自动执行。** PRD 锁定后，`/research` 控制器自动执行内部 compiler passes：Spec → Plan/TASK_QUEUE → paper draft。Spec/Plan/Paper 不再需要用户显式调用对应 skill；但每个 pass 后的 validation/audit gate 仍是硬门，不能自动跳过。之后进入持续循环：每轮从 `RESEARCH_SPINE.yaml` 识别 runnable RQ，并执行其 `rqs/RQxx/TASKS.yaml` 中的下一步任务；完成后运行 `update_state.py` 原子更新 `RESEARCH_SPINE.yaml`、RQ-local `TASKS.yaml`、兼容聚合视图 `TASK_QUEUE.yaml` 与状态文件。循环不询问用户，直到：
...
```text
默认路径：`/research` 先推进内部 Spec/Plan/Paper compiler passes；随后 Codex / Claude Code agent executor 读取 `Vn/RESEARCH_SPINE.yaml` 中的 non-final RQ 和对应 `rqs/RQxx/TASKS.yaml` 并提交结构化证据；`Vn/TASK_QUEUE.yaml` 仅作兼容聚合视图。
...
| [`research-status`](skills/research-status/SKILL.md) | 只读实验进展入口：报告当前目标、Big RQ、当前 gate、runnable RQ 集合、兼容 active task、RQ-local 进展、证据缺口、blocker、可继续任务与 validator 状态；不推进任务。 |
...
`ai_loop_prompt.md` 会包含 Claude Code `Subagent Dispatch` 段落。需要数学、文献、复现、编码、实验、分析、论文或审计工作时，主会话应优先委派 runtime-provided specialist worker / reviewer；本仓库只保证安装 `research-experiment.md` 这一实验执行模板。`/research` controller 仍负责 state、gate、promotion 和 audit 阻断。

## Research Goal

`research-goal` 生成或刷新当前 `Vn/goal.md`，并写入 `Vn/GOAL_LOCK.yaml`。它读取 `RESEARCH_DIRECTION.md`、`PRD.tex`、`BASELINE_LOCK.yaml`、`baselines/INDEX.yaml`、`RESEARCH_SPINE.yaml`、`rqs/`、`TASK_QUEUE.yaml`、`EVIDENCE_GATE.yaml` 和 `STATUS.yaml`，把这些合同压缩成面向 Codex / Claude Code goal mode 的长程目标。`goal.md` 不是单步行动 note，也不是 task queue；它必须覆盖每个 RQ 的计划/任务绑定、完整 task dependency graph、可并行分支和 blocked-branch 传播规则。RQ 调度真源是 `RESEARCH_SPINE.yaml`，单个 RQ 的执行真源是 `rqs/RQxx/TASKS.yaml`，`TASK_QUEUE.yaml` 只是兼容聚合视图。
