    ...
    def append_insight_entry(self, plan_id: str, status: str) -> None:
        ...
        entry = "\n".join(
            [
                ...
                "",
                "## Recommended Action",
                "- review implementation diff, harness output, stdout/stderr, and spec/plan delta before concluding the plan is blocked",
    ...
    def write_plan_next_step_prompt(self, plan_id: str) -> None:
        ...
        note = "\n".join(
            [
                ...
                "",
                "下一步：先 review implementation diff、harness 输出、stdout/stderr、artifact hash 和 spec/plan delta，判断是代码错误、harness 错误，还是思路/规格错误；随后读取 `plan.yaml` 中的最早 gate，运行声明 harness，保存 stdout/stderr、artifact hash，并更新 `run_log.md`、`blocker_log.md`、`decision_log.md` 与 `final_summary.md`。",
                "",
