...
- `docs/research/V0/scripts/pre_flight.sh`
...
- `V0/goal.md` is generated by `research-goal` as the per-version high-level execution prompt and constraint anchor. It defines the overall mission for the entire `V0`, every RQ's plan/task coverage, the version task dependency graph, the code-review-first triage rule for blocked tasks, and the blocker-note convention for `runs/TASK_XXX_blocker.md`; it is not a single-step action note. RQ 调度真源是 `RESEARCH_SPINE.yaml`，单个 RQ 的执行真源是 `rqs/RQxx/TASKS.yaml`，`TASK_QUEUE.yaml` 只是兼容聚合视图。
