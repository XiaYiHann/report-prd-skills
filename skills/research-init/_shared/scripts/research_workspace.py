...
- `plan_ready` / `running`：以 TASK_QUEUE 为调度真源，默认执行 active task；若 goal.md 标出的正交 runnable tasks 依赖满足且文件范围不冲突，可并行推进。
...
- 每轮开始必须读取 `TASK_QUEUE.yaml`；每个 task 的执行、run report 和 state update 都必须以该队列中的 task 条目为真源。
