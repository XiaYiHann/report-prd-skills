...
- `plan_ready` / `running`：以 `RESEARCH_SPINE.yaml` 作为 RQ 调度真源，以 `rqs/RQxx/TASKS.yaml` 作为单个 RQ 的执行真源；`TASK_QUEUE.yaml` 仅作兼容聚合视图。
...
你处于 Claude Code 目标模式中。每轮应读取持久化状态文件来推进研究。每轮至少执行一个 bounded RQ task；若执行器支持并行，可并行推进正交 runnable RQ tasks。
...
6. `docs/research/{CURRENT}/RESEARCH_SPINE.yaml` — 版本级 RQ 调度真源
7. `docs/research/{CURRENT}/rqs/RQxx/TASKS.yaml` — 单个 RQ 的执行真源
8. `docs/research/{CURRENT}/TASK_QUEUE.yaml` — 兼容聚合视图（success_criteria、test_commands、evidence_required）
...
### Step 3: Execute the selected RQ task
...
- 若任务需要 specialist worker / reviewer，优先使用 runtime specialist；若是声明实验执行，使用 `research-experiment`
...
2. `TASK_QUEUE.yaml` — 已执行 task status 改为 `done` 或 `blocked`；调度下一个 runnable unblocked task，并只冻结显式依赖 blocker 的后继分支
...
- Delegate bounded specialist work to the relevant runtime specialist or shipped experiment subagent when useful.
...
- 读取 `CURRENT`
- 管理 Vn 状态
- 执行或委派当前 runnable RQ 对应的 bounded task
- 判断是否 closeout
- 决定是否生成 Vn+1 draft
- 阻止 paper claim 越权
...
loop_mode:
  ...
  codex: goal_mode
active_task_source: RESEARCH_SPINE.yaml + rqs/RQxx/TASKS.yaml
loop_rules:
  - "RQ scheduling truth source is RESEARCH_SPINE.yaml; per-RQ execution truth source is rqs/RQxx/TASKS.yaml; TASK_QUEUE.yaml is only a compatibility aggregate view."
  - "Each loop selects runnable RQs from RESEARCH_SPINE.yaml and the next bounded task from the corresponding rqs/RQxx/TASKS.yaml."
  ...
  - "If no runnable RQ task exists, stop for human review or closeout."
...
GOAL_DONE_TASK_STATUSES = {"completed", "skipped", "done", "complete", "passed"}
...
def goal_task_rq_id(task: dict[str, Any]) -> str:
    ...
    return rq_id or "direction_bootstrap"


def goal_task_refs(task: dict[str, Any], rq_id: str) -> tuple[str, str]:
    if rq_id.startswith("RQ"):
        default_spec = f"rqs/{rq_id}/SPEC.yaml"
        default_task = f"rqs/{rq_id}/TASKS.yaml"
    else:
        default_spec = "n/a"
        default_task = "TASK_QUEUE.yaml"
    return str(task.get("rq_spec_ref") or default_spec), str(task.get("rq_task_ref") or default_task)
...
def goal_tasks_by_id(queue: dict[str, Any]) -> dict[str, dict[str, Any]]:
    return {
        goal_task_id(task): task
        for task in as_list(queue.get("tasks"))
        if isinstance(task, dict) and goal_task_id(task)
    }


def goal_dependents_map(queue: dict[str, Any]) -> dict[str, list[str]]:
    ...
    for task in as_list(queue.get("tasks")):
        ...
        for dep_id in goal_task_dependencies(task):
            dependents.setdefault(dep_id, []).append(task_id)
    return {task_id: sorted(children) for task_id, children in dependents.items()}


def goal_blocked_dependencies(
    task: dict[str, Any],
    ...
    def visit(dep_id: str) -> None:
        if dep_id in seen:
            return
        seen.add(dep_id)
        if status_by_id.get(dep_id) in GOAL_BLOCKED_TASK_STATUSES:
            blocked.add(dep_id)
        dep_task = tasks_by_id.get(dep_id)
        if isinstance(dep_task, dict):
            for parent_id in goal_task_dependencies(dep_task):
                visit(parent_id)

    for dependency_id in goal_task_dependencies(task):
        visit(dependency_id)
    return sorted(blocked)


def goal_waiting_dependencies(
    task: dict[str, Any],
    status_by_id: dict[str, str],
    blocked_by: list[str],
) -> list[str]:
    ...
    for dep_id in goal_task_dependencies(task):
        ...
        if status not in GOAL_DONE_TASK_STATUSES and dep_id not in blocked_set:
...
def goal_inline_list(items: list[Any], default: str = "none", limit: int = 4) -> str:
...
def goal_rq_execution_plan_lines(spine: dict[str, Any], queue: dict[str, Any]) -> str:
    task_ids_by_rq: dict[str, list[str]] = {}
    for task in as_list(queue.get("tasks")):
        if not isinstance(task, dict):
            continue
        task_id = goal_task_id(task)
        if not task_id:
            continue
        task_ids_by_rq.setdefault(goal_task_rq_id(task), []).append(task_id)

    lines: list[str] = []
    declared_rqs: set[str] = set()
    for rq in as_list(spine.get("research_questions")):
        if not isinstance(rq, dict):
            continue
        rq_id = str(rq.get("id") or "")
        if not rq_id:
            continue
        declared_rqs.add(rq_id)
        spec_ref = str(rq.get("spec_ref") or f"rqs/{rq_id}/SPEC.yaml")
        plan_ref = str(rq.get("plan_ref") or f"rqs/{rq_id}/PLAN.md")
        task_ref = f"rqs/{rq_id}/TASKS.yaml"
        queue_tasks = goal_inline_list(task_ids_by_rq.get(rq_id, []), default="none")
        lines.append(
            f"- `{rq_id}`: spec=`{spec_ref}`; plan=`{plan_ref}`; tasks=`{task_ref}`; "
            f"queue_tasks=`{queue_tasks}`"
        )
    for rq_id, task_ids in sorted(task_ids_by_rq.items()):
        if rq_id in declared_rqs:
            continue
        lines.append(f"- `{rq_id}`: queue_tasks=`{goal_inline_list(task_ids)}`")
    return "\n".join(lines) if lines else "- unresolved: `TASK_QUEUE.yaml` 尚未声明 task。"


def goal_outstanding_task_graph(queue: dict[str, Any]) -> str:
    status_by_id = goal_task_status_map(queue)
    tasks_by_id = goal_tasks_by_id(queue)
    dependents_by_id = goal_dependents_map(queue)
    lines: list[str] = []
    for task in as_list(queue.get("tasks")):
        if not isinstance(task, dict):
            continue
        ...
        rq_id = goal_task_rq_id(task)
        rq_spec_ref, rq_task_ref = goal_task_refs(task, rq_id)
        runnable = "yes" if goal_task_is_runnable(task, status_by_id) else "no"
        blocked_by = goal_blocked_dependencies(task, tasks_by_id, status_by_id)
        waiting_on = goal_waiting_dependencies(task, status_by_id, blocked_by)
        ...
        elif task_status in GOAL_DONE_TASK_STATUSES:
            parallel_class = "done"
        lines.append(
            f"- `{task_id}`: status=`{task_status}`; gate=`{task.get('gate_id', 'unknown')}`; "
            f"phase=`{task.get('phase', 'unknown')}`; rq=`{rq_id}`; what_to_do=`{task.get('title', '')}`; "
...
def goal_parallel_execution_lines(queue: dict[str, Any]) -> str:
    status_by_id = goal_task_status_map(queue)
    tasks_by_id = goal_tasks_by_id(queue)
    runnable: list[str] = []
    blocked_descendants: list[str] = []
    waiting: list[str] = []
    for task_id, task in tasks_by_id.items():
        status = str(task.get("status") or "")
        if status in GOAL_DONE_TASK_STATUSES:
            continue
        blocked_by = goal_blocked_dependencies(task, tasks_by_id, status_by_id)
        if blocked_by:
            blocked_descendants.append(f"{task_id}(blocked_by={goal_inline_list(blocked_by)})")
        elif goal_task_is_runnable(task, status_by_id):
            runnable.append(task_id)
        else:
            waiting_on = goal_waiting_dependencies(task, status_by_id, blocked_by)
            waiting.append(f"{task_id}(waiting_on={goal_inline_list(waiting_on)})")
    return "\n".join(
        [
            f"- runnable_parallel_set: `{goal_inline_list(sorted(runnable))}`",
            f"- blocked_descendants: `{goal_inline_list(sorted(blocked_descendants))}`",
            f"- waiting_on_dependencies: `{goal_inline_list(sorted(waiting))}`",
            "- parallel_rule: runnable tasks with satisfied `depends_on` edges may be executed in parallel only when their `allowed_files` do not conflict and each task still writes an independent run report and state update.",
            "- blocker_rule: a blocked task freezes only its explicit descendants; orthogonal runnable tasks must continue until no runnable unblocked task remains.",
        ]
    )
...
本文件由 `research-goal` 生成或刷新，是当前 `{version}` 的 version-level 长程执行锚点。它只定义任务边界、证据规则、gate 策略和停止条件；RQ 调度真源是 `RESEARCH_SPINE.yaml`，单个 RQ 的执行真源是 `rqs/RQxx/TASKS.yaml`，`TASK_QUEUE.yaml` 只是兼容聚合视图。
...
下面是当前 `{version}` 内每个 RQ 对应的计划、任务合同和全局队列绑定。`goal.md` 必须覆盖整个版本，不应退化为单步行动提示。

{rq_execution_plan}

## Outstanding Task Graph

以下是当前 `{version}` 的 version task dependency graph。它列出每个 task 该做什么、依赖什么、会解锁什么、当前是否可运行，以及属于可并行分支还是被阻塞后继分支。`depends_on` 是调度真源；`runnable_now=yes` 表示该 task 不依赖已阻塞分支，且可以在当前合同下继续推进。
...
- latest approved design source: use the current human instruction, human-approved `PRD.tex`, `RESEARCH_SPINE.yaml`, RQ-local `SPEC.yaml` / `PLAN.md` / `TASKS.yaml`, compatibility `TASK_QUEUE.yaml`, and `EVIDENCE_GATE.yaml` only when they agree on the same evidence boundary. If they conflict on research direction, core hypothesis, baseline, metric, dataset, or claim admission, request human review.
...
- validation_after_repair: run `goal-ready` after the drift repair, then continue executing the runnable RQ task set from `RESEARCH_SPINE.yaml` and `rqs/RQxx/TASKS.yaml`.
...
- 每轮开始必须读取 `RESEARCH_SPINE.yaml` 与相关 `rqs/RQxx/TASKS.yaml`；`TASK_QUEUE.yaml` 只用于兼容性的扁平聚合检查。
- 不允许从 `goal.md` 直接发明新 task；需要新 task 时必须由 RQ-local `PLAN.md` / `TASKS.yaml`、兼容聚合视图 `TASK_QUEUE.yaml` 或 audit repair route 生成。
- 若执行器支持并行工作，只有 `runnable_now=yes`、`allowed_files` 不冲突、且无共享 state update 冲突的 orthogonal tasks 可以并行；否则按依赖顺序串行执行。
...
- 当一个 RQ task 完成或阻塞后，调度器应优先继续执行 `RESEARCH_SPINE.yaml` 中其余 runnable unblocked RQ tasks；不要因为单个分支 blocker 直接停止整个版本。
...
- `BASELINE_LOCK.yaml`、`GOAL_LOCK.yaml`、`RESEARCH_SPINE.yaml`、RQ-local contracts、`EVIDENCE_GATE.yaml` 或兼容聚合视图 `TASK_QUEUE.yaml` stale，必须先刷新对应合同。
- 所有未完成 task 都不再 runnable，或剩余任务全部依赖已阻塞分支、缺失证据或显式人工决策；此时停止等待人类接管。
- 需要新增 `Vn+1`，但当前版本尚未 closeout。

## Subagent Execution Contract

- main controller remains responsible for state updates, gate decisions, final evidence admission, `update_state.py`, `LOOP_LOG.md`, `RESEARCH_SPINE.yaml`, RQ-local `TASKS.yaml`, compatibility `TASK_QUEUE.yaml`, `STATUS.yaml`, and `GOAL_LOCK.yaml`.
- prefer_subagents: 优先使用 subagent 执行 bounded specialist work；除非任务过小、没有匹配 runtime specialist、或下一步被主 controller 的状态更新阻塞，否则不要把文献、复现、编码、实验、分析、数学、论文或审计工作全部留在主会话内完成。
- Delegate bounded specialist work to subagents when the selected RQ task naturally matches a role; assign concrete `allowed_files`, expected artifacts, and validation commands, then review and integrate their result before state update.
- literature / benchmark / baseline selection -> runtime literature worker
- baseline reproduction -> runtime reproduction worker
- method or harness implementation -> runtime coding worker
- full experiment execution -> `research-experiment`
- analysis / anomaly / pivot -> runtime analysis reviewer
- math or formulation -> runtime math reviewer
- paper placeholder update -> internal paper compiler or runtime writing worker
- cross-file consistency, drift repair review, and gate check -> `/research audit` or runtime reviewer
...
每次长循环迭代结束时，用中文报告：完成的 task、修改的文件、运行的命令、测试或 blocker 证据、当前 gate 状态、后续 runnable task 集合。不要把 clean render 或 smoke test 当作科学结论。
"""
        ...
        "tasks": [
            {
                ...
                "agent_mode": ["main", "literature_scout"],
                "search": {
                    "required": True,
...
ALLOWED_PAPER_TYPES = {"method", "finding", "system", "benchmark", "survey"}
METHOD_PAPER_PROTECTION_DEFAULTS = {
...
def default_paper_type_payload(version: str) -> dict[str, Any]:
    return {
        ...
        "version": version,
        "paper_type": "method",
        "method_paper_protections": dict(METHOD_PAPER_PROTECTION_DEFAULTS),
...
def create_epoch(
    ...
    if pre_flight_template.exists():
        ...
        write_text(
            epoch_dir / "scripts" / "pre_flight.sh",
            "#!/usr/bin/env bash\n# pre_flight.sh template not found; copy from skills/research-init/_shared/templates/\n",
            force,
        )
    init_rq_scaffold(epoch_dir, version, "RQ01", force)
    ...
    write_text(
        ...
        "\n".join(
            [
                ...
                "若 Paper 与 Spec 冲突，以 Spec 为准。",
                "始终执行最早尚未完成的 gate。",
                "执行任何 task 前先检查 `RESEARCH_SPINE.yaml` 与对应 `rqs/RQxx/TASKS.yaml`；`TASK_QUEUE.yaml` 只作兼容聚合检查，缺失 binding 则停止并记录 blocker。",
                ...
                "## Subagent Dispatch",
                "",
                "When the current gate requires specialized work, delegate to the matching runtime specialist worker/reviewer when available:",
                "",
                "- mathematical formulation or proof issue → runtime math reviewer",
                "- literature / benchmark selection → runtime literature worker",
                "- baseline reproduction → runtime reproduction worker",
                "- method implementation → runtime coding worker",
                "- full experiment execution → `research-experiment`",
                "- result analysis / anomaly / pivot → runtime analysis reviewer",
                "- paper writing/update → internal paper compiler or runtime writing worker",
                "- cross-file consistency check → `/research audit` or runtime reviewer",
                "",
...
def validate_paper_type_payload(path: Path, payload: dict[str, Any]) -> list[str]:
    issues: list[str] = []
    label = path.name
    paper_type = str(payload.get("paper_type") or "")
    if paper_type not in ALLOWED_PAPER_TYPES:
        issues.append(f"{label} invalid paper_type: {paper_type or '<missing>'}")
    protections = payload.get("method_paper_protections")
    if paper_type == "method":
        if not isinstance(protections, dict):
            issues.append(f"{label} method_paper_protections must be a mapping for method paper_type")
        else:
            for key, expected in METHOD_PAPER_PROTECTION_DEFAULTS.items():
                if key not in protections:
                    issues.append(f"{label} method_paper_protections missing {key}")
                    continue
                observed = protections.get(key)
                if key == "claim_downgrade_protection":
                    if not isinstance(observed, bool):
                        issues.append(f"{label} method_paper_protections.{key} must be a boolean")
                    elif observed is not expected:
                        issues.append(f"{label} method_paper_protections.{key} must be {expected}")
                elif observed != expected:
                    issues.append(f"{label} method_paper_protections.{key} must be {expected}")
    tdd_required = payload.get("tdd_required")
    if not isinstance(tdd_required, dict):
        issues.append(f"{label} tdd_required must be a mapping")
    elif not isinstance(tdd_required.get("enabled"), bool):
        issues.append(f"{label} tdd_required.enabled must be a boolean")
    return issues
...
def epoch_rq_frontier(epoch_dir: Path, spine: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for rq in as_list(spine.get("research_questions")):
        ...
        for task in as_list(tasks_doc.get("tasks")):
            ...
            task_copy = dict(task)
            task_copy["_rq_id"] = rq_id
            task_copy["_rq_task_ref"] = f"rqs/{rq_id}/TASKS.yaml#{_task_identifier(task_copy)}"
            if next_task is None and status in {"active", "ready", "pending"}:
                next_task = task_copy
            if status in {"blocked", "failed", "error", "needs_human_review"}:
                blocked_tasks.append(task_copy)
        frontier.append({"rq_id": rq_id, "next_task": next_task, "blocked_tasks": blocked_tasks})
    return frontier


def validate_loop_ready_queue_projection(
    ...
    status_by_id = goal_task_status_map(load_yaml(epoch_dir / "TASK_QUEUE.yaml"))
...
def validate_epoch_schema(research_dir: Path, strict: bool = True) -> list[EpochSchemaIssue]:
    ...
    for epoch_dir in epoch_versions(research_dir):
        ...
        paper_type_path = epoch_dir / "PAPER_TYPE.yaml"
        if paper_type_path.exists():
            for issue in validate_paper_type_payload(paper_type_path, load_yaml(paper_type_path)):
                issues.append(EpochSchemaIssue(f"{version}/PAPER_TYPE.yaml", f"{version}/{issue}"))
...
def validate_loop_ready(research_dir: Path) -> Validation:
    ...
    tasks = [task for task in as_list(queue.get("tasks")) if isinstance(task, dict)]
    spine = load_yaml(epoch_dir / "RESEARCH_SPINE.yaml")
    rq_frontier = epoch_rq_frontier(epoch_dir, spine)
    rq_active = [
        item.get("next_task")
        for item in rq_frontier
        if isinstance(item.get("next_task"), dict) and str(item["next_task"].get("status") or "").lower() == "active"
    ]
    if rq_active:
        for rq_task in rq_active:
            rq_id = str(rq_task.get("_rq_id") or "")
            rq_task_ref = str(rq_task.get("_rq_task_ref") or "")
            matches = [
                task
                for task in tasks
                if str(task.get("rq_task_ref") or "") == rq_task_ref
            ]
            if not matches:
                validation.error(f"TASK_QUEUE.yaml missing compatibility projection for active RQ task: {rq_task_ref}")
                continue
            projection = matches[0]
            if str(projection.get("status") or "").lower() != "active":
                validation.error(f"TASK_QUEUE.yaml compatibility projection is stale for active RQ task: {rq_task_ref}")
            binding = projection.get("research_binding") if isinstance(projection.get("research_binding"), dict) else {}
            projection_rq_id = str(projection.get("rq_id") or binding.get("rq_id") or "")
            if projection_rq_id and rq_id and projection_rq_id != rq_id:
                validation.error(f"TASK_QUEUE.yaml compatibility projection rq_id mismatch for {rq_task_ref}: {projection_rq_id}")
            validate_loop_ready_queue_projection(validation, projection, epoch_dir)
        return validation

    active = [task for task in tasks if str(task.get("status") or "").lower() == "active"]
    if len(active) != 1:
        validation.error("TASK_QUEUE.yaml aggregate view must declare exactly one active task when no RQ-local active task exists")
        return validation
    validate_loop_ready_queue_projection(validation, active[0], epoch_dir)
    return validation
...
def validate_goal_ready(research_dir: Path) -> Validation:
    ...
    required_truth_clause = "RQ 调度真源是 `RESEARCH_SPINE.yaml`，单个 RQ 的执行真源是 `rqs/RQxx/TASKS.yaml`，`TASK_QUEUE.yaml` 只是兼容聚合视图。"
    if required_truth_clause not in goal_text:
        validation.error("goal.md must state the RESEARCH_SPINE -> RQ TASKS -> TASK_QUEUE authority hierarchy")
    for required in [
        "RQ Contract Coverage",
        "RQ Execution Plan Matrix",
        "Outstanding Task Graph",
